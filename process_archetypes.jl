...
if length(ARGS) < 6
    ...
    10. `-results <definitions>`, the url to a Spine Datastore where the baseline results are to be written. If not provided, results are written into the definitions url.
    11. `-weather <definitions>`, the url of the Spine Datastore into which the autogenerated weather data is imported. If not provided, weather data is written  into the definitions url.
    12. `-save_layouts <false>`, controls whether auto-generated `building_weather` layouts are saved as images.
    13. `-alternative <"">`, the name of the alternative where the parameters are saved, empty by default.
    14. `-realization <realization>`, The name of the stochastic scenario containing true data over forecasts, only relevant when generating stochastic forecast data.
    """
